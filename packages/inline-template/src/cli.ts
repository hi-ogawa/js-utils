...
const cli = new TinyCliCommand(
  ...
  async ({ args }) => {
    ...
    const output = processInput(input, { cwd: args.cwd });
    if (args.inplace) {
...
function processInput(input: string, options: TemplateOpitons): string {
  // collect templates
  ...
  // apply templates
  for (const id of ids) {
    input = applyTemplate(input, id, options);
  }
...
function applyTemplate(
  ...
  const templateOut = applyInterpolation(templateIn, options);
  const output = input.slice(0, outBegin) + templateOut + input.slice(outEnd);
  return output;
}

function applyInterpolation(input: string, options: TemplateOpitons): string {
